...
object Base {
  def main(args: Array[String]): Unit = {
     ...
     *  scala解释器也被称为REPL，会快速编译scala代码为字节码，然后交给JVM来执行。
		 ...
		 *  如果你没有指定变量来存放这个值，那么值默认的名称为res，而且会显示结果的数据类型，比如Int、Double、String等。
     ...
     */
    println("“Hello”.intersect(“World”):" + "Hello".intersect("World"))
    println(1.to(10))
    ...
    println("“Hello World”.distinct:" + "Hello World".distinct)
    
    /**
     * ·apply函数
     * ·Scala中的apply函数是非常特殊的一种函数，在Scala的object中，可以声明apply函数。而使用“类名()”的形式，其实就是“类名.apply()”的一种缩写。通常使用这种方式来构造类的对象，而不是使用“new 类名()”的方式。
     * ·例如，“Hello World”(6)，因为在StringOps类中有def apply(n: Int): Char的函数定义，所以“Hello World”(6)，实际上是“Hello World”.apply(6)的缩写，结果是W（即从0开始截取了第6个字符）。
     * ·例如，Array(1, 2, 3, 4)，实际上是用Array object的apply()函数来创建Array类的实例，也就是一个数组。
     * 
     */
    println("“Hello World”(6):"+"Hello World"(6))
    println("“Hello World”.apply(6):" + "Hello World".apply(6))
    Array(1,2,3,4).foreach { println(_)}
  }
}