...
object PatternMatch {
  def main(args: Array[String]): Unit = {
    ...
    //但是Scala的模式匹配除了可以对值进行匹配之外，还可以对类型进行匹配、对Array和List的元素情况进行匹配、对case class进行匹配、甚至对有值或没值（Option）进行匹配。
    ...
    greeting(Array("Leo", "leo3", "leo2", "leo4"))
    greeting(List("Leo", "leo1", "loe2"))
    //4、case class与模式匹配（案例：学校门禁）
    judgeIdentify(Teacher("Leo", "数学"))
    judgeIdentify(Student("Leo", "339"))
